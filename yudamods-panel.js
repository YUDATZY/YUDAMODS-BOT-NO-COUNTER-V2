...
const nama = 'YudaMods - Vip';
const author = 'YudaMods - Vip';
// Informasi waktu mulai bot
const startTime = Date.now();
//▰▰▰▰▰▰▰▰▰▰▰▰▰▰▰▰▰▰▰▰▰▰▰//
// start
bot.onText(/\/panelmenu/, (msg) => {
...
bot.on('callback_query', (callbackQuery) => {
  if (callbackQuery.data === 'myprofil') {
    ...
    bot.sendMessage(callbackQuery.from.id, 'Hallo, saya adalah My Profil. Bot Ini Milik @YUDAMODS');
  }
});
// Event listener for button 'Start'
bot.on('callback_query', (callbackQuery) => {
    if (callbackQuery.data === 'panelmenu') {
...
• 10GB ( PREMIUM ) ✅
• UNLI ( PREMIUM ) ✅ 
🛑 JOIN RESELLER CUMA 6K BANG 😁

𝗕𝘂𝘆 𝗣𝗿𝗲𝗺? 𝗕𝘂𝘆 𝗩𝗽𝘀? 𝗕𝘂𝘆 𝗔𝗱𝗺𝗶𝗻𝗣&𝗣𝘁 𝗣𝗮𝗻𝗲𝗹? 𝗕𝘂𝘆 𝗦𝗰? 𝗣𝘃 (@YUDAMODS)`;
    const keyboard = {
        reply_markup: {
            inline_keyboard: [
                [{ text: '🖥️ Buy Panel', url: 'https://t.me/YUDAMODS/buy_panel' }, { text: '👤 Buy Admin', url: 'https://t.me/YUDAMODS/buyadminp & ptpanel' }],
                [{ text: '🇲🇨 Buy Vps', url: 'https://t.me/YUDAMODS/buyvps' }]
            ]
        }
    };
    bot.sendPhoto(chatId, config.pp, { caption: text12, parse_mode: 'Markdown', reply_markup: keyboard });
});
//▰▰▰▰▰▰▰▰▰▰▰▰▰▰▰▰▰▰▰▰▰▰▰//
// ramlist2
const message = menuText;
...
bot.on('callback_query', (callbackQuery) => {
  if (callbackQuery.data === 'ramlist') {
    ...
    bot.editMessageText(ramListMessage, {
      ...
      reply_markup: {
        inline_keyboard: [
          [{ text: 'Kembali ke Menu Panel', callback_data: 'panelmenu' }]
        ]
      }
    });
  }
